...
def eval_one_epoch_channel(
    ...
    """
    Local evaluation helper for channel ablation.

    The public evaluator uses the model default n_mc. This helper keeps the
    same metric keys but allows the final stable test evaluation to use
    n_mc=500 without changing the training-time settings.
    """
    model.eval()
...
def eval_final_test_channel(
    ...
    """
    Final-test-only evaluator.

    Important:
    - No teacher forcing for the hierarchical prediction path.
    - SOH receives predicted SOC because model.forward() is called with
      soc_tf=None.
    - Training, validation, checkpoint selection, and resume behavior are not
    ...
    for x_img, x_pt, y_cls, soc_tf, soh_tf in loader:
        ...
        # FINAL TEST ONLY: predicted SOC is passed downstream to SOH.
        ...
        if float(w_soc) > 0:
            soc_reg = criterion_reg(
                soc_pred.view(-1),
                soc_tf.view(-1),
            ).mean()

            # Ground-truth SOC is used only to evaluate SOC likelihood.
            # It is NOT passed into the downstream SOH flow.
            soc_logp_eval = model.soc_flow.log_prob(
                soc_tf.view(-1),
                cond_soc,
            ).view(-1)
            soc_nll = -soc_logp_eval.mean()
            soc_reg = soc_reg + float(soc_nll_weight) * soc_nll

            loss = loss + float(w_soc) * soc_reg

        if float(w_soh) > 0:
            soh_reg = criterion_reg(
                soh_pred.view(-1),
                soh_tf.view(-1),
            ).mean()
            loss = loss + float(w_soh) * soh_reg
        ...
        y_true_all.append(y_cls.detach().cpu().numpy())
        y_pred_all.append(
            torch.argmax(logits_mat, dim=1).detach().cpu().numpy()
        )
    ...
    if total_n <= 0:
        raise RuntimeError("Empty dataloader in final test evaluation.")
...
def run_channel_experiment(
    ...
    if summary_only:
        ...
        drop2d = float(cfg.get("drop2d", drop2d))
...
def _add_summary_columns(summary: pd.DataFrame) -> pd.DataFrame:
    ...
    summary["mat_acc_pct"] = summary["fine_acc_pct"]
    ...
    if "full" in set(summary["channel_mode"]):
        ...
        summary["fine_acc_change_pp_vs_full"] = (summary["test_cls_acc"].astype(float) - float(ref["test_cls_acc"])) * 100.0
        summary["mat_acc_change_pp_vs_full"] = summary["fine_acc_change_pp_vs_full"]
        summary["material_acc_change_pp_vs_full"] = (summary["test_material_acc"].astype(float) - float(ref["test_material_acc"])) * 100.0
        summary["soc_medae_change_pp_vs_full"] = summary["test_soc_medae_raw"].astype(float) - float(ref["test_soc_medae_raw"])
        summary["soh_medae_change_pp_vs_full"] = summary["test_soh_medae_raw"].astype(float) - float(ref["test_soh_medae_raw"])
        summary["soc_medape_change_pp_vs_full"] = summary["test_soc_medape_raw"].astype(float) - float(ref["test_soc_medape_raw"])
        summary["soh_medape_change_pp_vs_full"] = summary["test_soh_medape_raw"].astype(float) - float(ref["test_soh_medape_raw"])
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train or re-evaluate channel-ablation models.")
    parser.add_argument("--summary-only", action="store_true", help="Skip training and evaluate existing best.pt checkpoints only.")
    parser.add_argument("--smoke", action="store_true", help="Use the smoke-test configuration.")
    parser.add_argument("--no-resume", action="store_true", help="Disable checkpoint resume when training.")
    parser.add_argument("--data-root", type=Path, default=PROJECT_ROOT / "data")
    parser.add_argument("--output-root", type=Path, default=PROJECT_ROOT / "results" / "ablation" / "channel_ablation")
