...
def _add_summary_columns(summary: pd.DataFrame) -> pd.DataFrame:
    ...
    if "hierarchical" in set(summary["struct_mode"]):
        ...
        summary["mat_acc_change_pp_vs_hierarchical"] = (
            summary["test_cls_acc"].astype(float) - ref_acc
        ) * 100.0

        summary["soc_medape_change_pp_vs_hierarchical"] = (
            summary["test_soc_medape_raw"].astype(float) - ref_soc
        )

        summary["soh_medape_change_pp_vs_hierarchical"] = (
...
def run_hierarchy_ablation(
    ...
    if smoke:
        struct_modes = ["independent", "soc_to_soh", "hierarchical"]
        pulse_list = [5000]
