...
from proposed_framework.run_proposed_framework import run_experiment as run_structured_experiment
...
def _add_summary_columns(summary: pd.DataFrame) -> pd.DataFrame:
    ...
    if "structured_3ch" in set(summary["input_representation"]):
        ref = summary.loc[
            summary["input_representation"] == "structured_3ch"
        ].iloc[0]
        ...
        summary["mat_acc_change_pp_vs_structured"] = (
            summary["test_cls_acc"].astype(float) - ref_acc
        ) * 100.0

        summary["soc_medape_change_pp_vs_structured"] = (
            summary["test_soc_medape_raw"].astype(float) - ref_soc
        )

        summary["soh_medape_change_pp_vs_structured"] = (
...
def run_input_representation_ablation(
    ...
    structured_3ch:
        U1-U41 -> 3x5x8 structured representation -> proposed framework.
    """
    data_root = Path(data_root)
    ...
    if run_structured:
        structured_exp_dir = output_root / "structured_3ch"

        print("\n" + "=" * 90)
        print("[RUN] Input representation: structured_3ch")
        print(f"[RUN] Pulse list: {pulse_list}")
        print(f"[RUN] Output directory: {structured_exp_dir}")
        print("=" * 90)

        structured_out = run_structured_experiment(
            data_root=str(data_root),
            pulse_list=pulse_list,
...
def main() -> None:
    ...
    output_root = PROJECT_ROOT / "results" / "ablation" / "input_representation"
