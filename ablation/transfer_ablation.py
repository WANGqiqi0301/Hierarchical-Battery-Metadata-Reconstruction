...
TRANSFER_MODES: Dict[str, str] = {
    "soft": "Use material softmax probability vector as downstream context.",
    "hard": "Use argmax material prediction converted to one-hot downstream context.",
}
...
class TransferAblationModel(nn.Module):
    """
    Soft-vs-hard material-transfer ablation model.

    Difference between modes
    ------------------------
    soft:
        p_material = softmax(logits)
        The full material probability vector is passed to SOC and SOH heads.

    hard:
        p_material = one_hot(argmax(softmax(logits)))
        Only the most likely material class is passed to SOC and SOH heads.

    The rest of the hierarchy follows the proposed order:
        Material -> SOC -> SOH
    """
    ...
    def __init__(
        ...
        material_condition_mode = str(material_condition_mode).lower()

        if material_condition_mode not in TRANSFER_MODES:
            raise ValueError(
                f"material_condition_mode must be one of {list(TRANSFER_MODES)}."
            )

        self.material_condition_mode = material_condition_mode
        ...
        # Keep material classification consistent with the updated organized code:
        # material head can use encoder embedding plus pulse-width feature.
    ...
    def _material_context(self, logits_mat: torch.Tensor) -> torch.Tensor:
        """
        Convert material logits into downstream material context.

        soft:
            full probability vector.

        hard:
            one-hot vector from predicted argmax class.
        """
        ...
        if self.material_condition_mode == "hard":
            hard_idx = torch.argmax(p_soft, dim=1)
            return torch.zeros_like(p_soft).scatter_(1, hard_idx.unsqueeze(1), 1.0)
    ...
    def forward(
        ...
        p_mat = self._material_context(logits_mat)
...
def run_transfer_experiment(
    ...
    """
    Train and evaluate one material-transfer mode.
    """
    material_condition_mode = str(material_condition_mode).lower()
    ...
    print(f"[INFO] Material transfer mode: {material_condition_mode}")
    print(f"[INFO] Meaning: {TRANSFER_MODES[material_condition_mode]}")

    run_config = {
        "material_condition_mode": material_condition_mode,
        "description": TRANSFER_MODES[material_condition_mode],
    ...
    def run_stage(
        ...
        for epoch in range(start_epoch, int(epochs)):
            ...
            print(
                f"[{material_condition_mode} | {stage}] epoch {epoch:03d} | "
            ...
            if early_stopping and bad_count >= patience:
                print(
                    f"[EARLY STOP-{material_condition_mode} | {stage}] "
        ...
        if best_path.exists():
            ...
            print(
                f"[{material_condition_mode} | {stage}] Loaded BEST checkpoint from "
...
def _add_summary_columns(summary: pd.DataFrame) -> pd.DataFrame:
    ...
    if "soft" in set(summary["material_condition_mode"]):
        ...
        summary["mat_acc_change_pp_vs_soft"] = (
...
def run_transfer_ablation(
    ...
    """
    Run soft-vs-hard material-transfer ablation.

    soft:
        material probability vector is passed downstream.

    hard:
        predicted material class is converted into one-hot vector and passed downstream.
    """
    data_root = Path(data_root)
    output_root = Path(output_root)
    output_root.mkdir(parents=True, exist_ok=True)

    shared_cache_dir = output_root / "cache"
    ...
    for mode in modes:
        ...
        print("\n" + "=" * 90)
        print(f"[RUN] Transfer ablation: {mode}")
        print(f"[RUN] Meaning: {TRANSFER_MODES[mode]}")
        ...
        partial.to_csv(
            output_root / "transfer_ablation_partial.csv",
            index=False,
            encoding="utf-8-sig",
        )

        _save_json(
            output_root / "transfer_ablation_partial.json",
            rows,
        )
    ...
    summary.to_csv(
        output_root / "transfer_ablation_summary.csv",
        index=False,
        encoding="utf-8-sig",
    )

    _save_json(
        output_root / "transfer_ablation_summary.json",
...
def main() -> None:
    ...
    output_root = PROJECT_ROOT / "results" / "ablation" / "transfer_ablation"
