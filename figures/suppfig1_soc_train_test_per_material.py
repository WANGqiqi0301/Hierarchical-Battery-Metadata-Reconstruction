# -*- coding: utf-8 -*-
"""
suppfig1_soc_train_test_per_material.py

Supplementary Figure 1:
SOC train-test distribution by material-capacity group.
...
Notes:
1. Raw Excel data are used because this figure shows train/test SOC distributions,
...
def find_soc_column(df: pd.DataFrame):
    for c in df.columns:
        if str(c).strip() == SOC_COL:
            return c

    for c in df.columns:
        c_norm = str(c).strip().lower().replace(" ", "")
        if c_norm in ["soc", "soc(%)", "soc/%", "soc_percent", "socpercent"]:
            return c
...
def parse_soc_from_sheet_name(sheet_name: str):
    m = re.match(r"^\s*SOC\s*(\d+(?:\.\d+)?)\s*$", str(sheet_name), re.IGNORECASE)
    if m:
        return float(m.group(1))
    return None


def normalize_soc_to_percent(values):
...
def read_one_sheet_soc(
    ...
    if soc_col is not None:
        soc_values = pd.to_numeric(df[soc_col], errors="coerce").to_numpy(dtype=float)
        soc_values = normalize_soc_to_percent(soc_values)
        out[SOC_COL] = soc_values
        ...
        raise KeyError(
            f"{file_path} | sheet={sheet_name}: missing SOC column and cannot parse SOC "
            f"from sheet name. Available columns: {list(df.columns)}"
        )

    out[SOC_COL] = pd.to_numeric(out[SOC_COL], errors="coerce")
    out = out.dropna(subset=[ID_COL, SOC_COL]).copy()
...
def plot_soc_train_bar_density(
    ax,
    values,
    bar_color,
    kde_color,
    bar_label="Train SOC grid",
    kde_label="Train KDE",
):
    values = np.asarray(values, dtype=float)
    values = values[np.isfinite(values)]
    ...
    dens = cnt / (len(values) * TRAIN_SOC_GRID_STEP)

    ax.bar(
        uniq,
        dens,
        width=TRAIN_BAR_WIDTH,
        color=bar_color,
        alpha=GLOBAL_ALPHA,
        edgecolor="none",
        label=bar_label,
    ...
    if len(values) >= 2 and np.nanstd(values) > 1e-12:
        ...
        xs = np.linspace(SOC_XLIM[0], SOC_XLIM[1], 400)
        ax.plot(
            xs,
            kde(xs),
            color=kde_color,
...
def plot_soc_test_hist_kde_density(
    ax,
    values,
    bins,
    bar_color,
...
def _calc_ymax_for_group(df_g: pd.DataFrame) -> float:
    y_max = 0.0
    bins = np.linspace(SOC_XLIM[0], SOC_XLIM[1], N_BINS_TEST + 1)

    train_values = df_g.loc[df_g["split"] == "train", SOC_COL].to_numpy(dtype=float)
    train_values = train_values[np.isfinite(train_values)]

    if len(train_values) > 0:
        _, cnt = np.unique(train_values, return_counts=True)
        dens = cnt / (len(train_values) * TRAIN_SOC_GRID_STEP)

        if len(dens) > 0 and np.isfinite(dens).any():
            y_max = max(y_max, np.nanmax(dens))

        if len(train_values) >= 2 and np.nanstd(train_values) > 1e-12:
            kde = gaussian_kde(train_values)
            xs = np.linspace(SOC_XLIM[0], SOC_XLIM[1], 400)
            y_max = max(y_max, np.nanmax(kde(xs)))

    test_values = df_g.loc[df_g["split"] == "test", SOC_COL].to_numpy(dtype=float)
    test_values = test_values[np.isfinite(test_values)]

    if len(test_values) > 0:
        hist_density, _ = np.histogram(test_values, bins=bins, density=True)

        if len(hist_density) > 0 and np.isfinite(hist_density).any():
            y_max = max(y_max, np.nanmax(hist_density))

        if len(test_values) >= 2 and np.nanstd(test_values) > 1e-12:
            kde = gaussian_kde(test_values)
            xs = np.linspace(SOC_XLIM[0], SOC_XLIM[1], 400)
            y_max = max(y_max, np.nanmax(kde(xs)))
...
def compute_mixed_ylim(data: pd.DataFrame):
    ...
    for g in other_groups:
        ...
        y_max_global = max(y_max_global, _calc_ymax_for_group(df_g))
    ...
    for g in nmc_groups:
        df_g = data.loc[data["material_capacity"] == g].copy()
        y_max = _calc_ymax_for_group(df_g)
...
def save_plot_for_group(
    ...
    train_values = df_group.loc[df_group["split"] == "train", SOC_COL].to_numpy(dtype=float)
    test_values = df_group.loc[df_group["split"] == "test", SOC_COL].to_numpy(dtype=float)
    ...
    plot_soc_test_hist_kde_density(
        ax=ax,
        values=test_values,
        bins=bins,
    ...
    ax.set_xlim(SOC_XLIM)
    ax.set_ylim(y_limit)

    ax.set_title(f"{material_capacity} SOC Train-Test Distribution", fontsize=14)
    ...
    save_path = SAVE_DIR / f"{file_prefix}_SOC_train_test.png"
...
# =========================================================
def main():
    print("[INFO] Generating Supplementary Figure 1...")
    ...
    before_n = len(data)
    data = data[np.isfinite(data[SOC_COL].to_numpy(dtype=float))].copy()
    data = data[(data[SOC_COL] >= 0) & (data[SOC_COL] <= 100)].copy()
    after_n = len(data)

    if after_n < before_n:
        print(f"[WARN] Removed invalid SOC rows: {before_n - after_n}")

    global_ylim, nmc_ylim_dict = compute_mixed_ylim(data)

    raw_path = SAVE_DIR / "suppfig1_SOC_train_test_raw_values.csv"
    data.to_csv(raw_path, index=False, encoding="utf-8-sig")

    summary = (
        data.groupby(["material_capacity", "split"])[SOC_COL]
        .agg(["count", "min", "max", "mean", "median"])
        .reset_index()
    )

    summary_path = SAVE_DIR / "suppfig1_SOC_train_test_summary.csv"
    ...
    for g in groups:
        ...
        save_plot_for_group(df_g, g, y_limit)
