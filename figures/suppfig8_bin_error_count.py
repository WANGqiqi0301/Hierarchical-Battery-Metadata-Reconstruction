...
1. SOC/SOH absolute error (AE) from true and predicted values
2. SOC/SOH bins from true values
3. Bin-wise sample count
4. Bin-wise median absolute error (MedAE)
5. Bin-wise mean absolute error (MAE)
6. Bootstrap 95% CI of MedAE
...
def normalize_to_percent(values: pd.Series | np.ndarray) -> np.ndarray:
    """
    Convert values to percentage-point scale when they appear to be stored
    on a 0-1 scale. Values already on a 0-100 scale are left unchanged.
    """
...
def bootstrap_median_ci(
    ...
    """
    Bootstrap confidence interval for the median absolute error.
    """
...
def add_bins(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # --------------------------------------------------------
    # Convert reporting values to percentage-point scale
    # --------------------------------------------------------
    df["soc_true_pct"] = normalize_to_percent(
        df["soc_true"]
    )

    df["soc_pred_pct"] = normalize_to_percent(
        df["soc_pred"]
    )

    df["soh_true_pct"] = normalize_to_percent(
        df["soh_true"]
    )

    df["soh_pred_pct"] = normalize_to_percent(
        df["soh_pred"]
    )
    ...
    if SOH_BIN_RANGE is None:
        ...
        soh_values = soh_values[
            np.isfinite(soh_values)
        ]

        if len(soh_values) == 0:
            raise RuntimeError(
                "No valid SOH values found for binning."
            )
...
def summarize_by_bin(
    ...
    """
    Summarize absolute error within each SOC/SOH bin.

    median -> MedAE
    mean   -> MAE
    ci_lo / ci_hi -> bootstrap 95% CI of MedAE
    """
    rows = []
    ...
    for bin_interval, group in grouped:
        ...
        rows.append({
            ...
            # Primary metric: MedAE
            "medae": float(np.median(values)),

            # Companion metric: MAE
            "mae": float(np.mean(values)),

            # Distribution summaries of absolute error
            "q25_ae": float(np.percentile(values, 25)),
            "q75_ae": float(np.percentile(values, 75)),

            # Bootstrap 95% CI of MedAE
            "medae_ci_lo": float(ci_lo),
            "medae_ci_hi": float(ci_hi),
        })
    ...
    if summary.empty:
        raise RuntimeError(
            f"No valid rows found when summarizing {bin_col}."
        )
...
def main():
    print(
        "[INFO] Generating Supplementary Figure 8..."
    )
    ...
    print(
        "\n[DONE] Supplementary Figure 8 generated."
    )

    print(
        f"[SAVED] Figure PNG: {png_path}"
    )

    print(
        f"[SAVED] Figure PDF: {pdf_path}"
    )

    print(
        f"[SAVED] SOC summary: {soc_summary_path}"
    )

    print(
        f"[SAVED] SOH summary: {soh_summary_path}"
    )
