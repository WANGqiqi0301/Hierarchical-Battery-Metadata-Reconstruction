# -*- coding: utf-8 -*-
"""
figures/plot_fig3b_material_confusion_3x3.py

Reproduce the top panel of Figure 3b:
3 x 3 material confusion matrix on the test set.

This simplified version:
1. Generates or loads per-sample 8-class predictions.
2. Maps each class label to its material (LFP, LMO, or NMC).
3. Computes the 3 x 3 row-normalized material confusion matrix.
4. Saves complete and pure PNG figures.

Example using old i10 results:

    python figures/plot_fig3b_material_confusion_3x3.py ^
        --data_root data ^
...
def get_material(label: str) -> str:
    """
    Convert a complete material-capacity label to material only.
...
def compute_confusion_data(df: pd.DataFrame):
    """
    Compute the 3 x 3 material confusion matrix.

    The original model prediction remains the complete 8-class prediction.
    Material-level evaluation is obtained by collapsing both the true and
    predicted class labels to their material prefix.
    """
    ...
    if missing_columns:
        raise RuntimeError(
            f"Prediction table missing columns: {missing_columns}"
        )

    y_true_cls = (
        pd.Series(df["true_label"])
        .astype(str)
    )

    y_pred_cls = (
        pd.Series(df["pred_label"])
        .astype(str)
    )

    y_true_material = y_true_cls.map(get_material)
    y_pred_material = y_pred_cls.map(get_material)
    ...
    if unknown_materials:
        raise RuntimeError(
            "Unexpected material labels found after collapsing classes: "
            f"{unknown_materials}. Expected only {MATERIAL_ORDER}."
        )

    labels = MATERIAL_ORDER.copy()
...
def plot_confusion_matrix_png(
    ...
    ax.set_title(f"Material confusion matrix (Accuracy: {accuracy:.2%})")
    ax.set_xlabel("Predicted material")
    ax.set_ylabel("True material")
...
def plot_confusion_matrix_pure_png(
    cm_normalized: np.ndarray,
    out_path: str | Path,
    cfg: dict,
    dpi: int,
    ...
    """
    Save a pure heatmap version:
    no axes, no ticks, no labels, no title, no annotations, and no colorbar.
    Only the confusion-matrix graphic itself is retained.
    """
    out_path = Path(out_path)
    ensure_dir(out_path.parent)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Plot Figure 3b 3x3 material confusion matrix."
    )
    ...
    parser.add_argument(
        "--prediction_csv",
        type=str,
        default="results/proposed_framework/further_analysis/tables/test_predictions_per_sample.csv",
    )

    parser.add_argument(
        "--fig_out_path",
        type=str,
        default="results/figures/main/fig3b/fig3b_material_confusion_3x3.png",
    )

    parser.add_argument(
        "--pure_out_path",
        type=str,
        default="results/figures/main/fig3b/fig3b_material_confusion_3x3_pure.png",
    )
...
def main():
    ...
    print("[FIGURE] Figure 3b 3x3 material confusion matrix")
    ...
    labels, cm, cm_normalized, accuracy = compute_confusion_data(df)

    # Save the numerical matrices for direct checking and reporting.
    matrix_dir = ensure_dir(
        Path(args.fig_out_path).parent
    )

    count_csv = (
        matrix_dir
        / "fig3b_material_confusion_3x3_count.csv"
    )

    normalized_csv = (
        matrix_dir
        / "fig3b_material_confusion_3x3_normalized.csv"
    )
    ...
    plot_confusion_matrix_pure_png(
        cm_normalized=cm_normalized,
    ...
    print("\n" + "=" * 70)
    print("[3x3 MATERIAL CONFUSION MATRIX]")
    print("=" * 70)
    ...
    print(f"\n[METRIC] Material accuracy = {accuracy:.4%}")
    print(f"[SAVED] Count matrix: {count_csv}")
    print(f"[SAVED] Normalized matrix: {normalized_csv}")
    print("[DONE] Complete and pure 3x3 material confusion-matrix PNG files saved successfully.")
