# -*- coding: utf-8 -*-
"""
plot_fig2f_delta_u_evolution.py

Figure 2f:
Delta-U evolution with SOC and SOH.

Only standard annotated figures are saved.

Outputs:
    results/figures/main/fig2f/Fig2f_DeltaU_SOC_Evolution_Standard.png
    results/figures/main/fig2f/Fig2f_DeltaU_SOH_Evolution_Standard.png
"""
...
# Plots
# =============================================================================
def plot_delta_u_soc_evolution(X: np.ndarray, soc: np.ndarray, soh: np.ndarray) -> None:
    X_delta = X[:, 1:] - X[:, :-1]
    ...
    ax.set_title("A. Differential Voltage Response vs SOC", fontweight="bold")
    ax.set_xlabel("Feature Index (Delta U2 - Delta U41)")
    ax.set_ylabel("Delta U (V)")
    ax.legend()
    ax.grid(True, linestyle="--", alpha=0.4)

    save_standard(fig, "Fig2f_DeltaU_SOC_Evolution_Standard.png")


def plot_delta_u_soh_evolution(X: np.ndarray, soc: np.ndarray, soh: np.ndarray) -> None:
    X_delta = X[:, 1:] - X[:, :-1]
    x_axis = np.arange(2, 42)
    ...
    ax.set_title(
        f"B. Differential Polarization Expansion (SOC approx. {best_soc:.0f}%)",
        fontweight="bold",
    )
    ax.set_xlabel("Feature Index (Delta U2 - Delta U41)")
    ax.set_ylabel("Delta U (V)")
    ...
    save_standard(fig, "Fig2f_DeltaU_SOH_Evolution_Standard.png")
...
def main() -> None:
    ...
    plot_delta_u_soc_evolution(X, soc, soh)
    plot_delta_u_soh_evolution(X, soc, soh)

    print("[DONE] Figure 2f Delta-U standard plots generated.")
