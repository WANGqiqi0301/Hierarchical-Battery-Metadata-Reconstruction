...
Reproduce Figure 3b second panel: SOC MedAE heatmap.
...
def ensure_soc_error_columns(df: pd.DataFrame) -> pd.DataFrame:
    """
    Ensure that the prediction table contains the SOC absolute-error column
    required for MedAE analysis.

    SOC values are expected in reporting units (percentage points). If the
    existing table already contains soc_ae, it is reused; otherwise soc_ae is
    calculated directly from soc_true and soc_pred.
    """
    df = df.copy()
    ...
    if "soc_ae" in df.columns:
        df["ae"] = pd.to_numeric(df["soc_ae"], errors="coerce")
    elif {"soc_true", "soc_pred"}.issubset(df.columns):
        soc_true = pd.to_numeric(df["soc_true"], errors="coerce")
        soc_pred = pd.to_numeric(df["soc_pred"], errors="coerce")
        df["soc_ae"] = np.abs(soc_pred - soc_true)
        df["ae"] = df["soc_ae"]
    else:
        raise RuntimeError(
            "Prediction table must contain soc_ae or soc_true/soc_pred."
        )

    if "soc_bin" not in df.columns:
        if "soc_true" not in df.columns:
            raise RuntimeError("Prediction table must contain soc_true to build SOC bins.")
        df["soc_bin"] = pd.cut(
            pd.to_numeric(df["soc_true"], errors="coerce"),
            bins=10,
        )
...
def build_soc_medae_pivot(df: pd.DataFrame) -> pd.DataFrame:
...
def plot_soc_medae(
    ...
    pivot = build_soc_medae_pivot(df)
    mask = pivot.isnull()
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Plot Figure 3b SOC MedAE heatmap."
    )
    ...
    parser.add_argument(
        "--fig_out_path",
        type=str,
        default="results/figures/main/fig3b/fig3b_soc_medae.png",
    )

    parser.add_argument(
        "--pure_out_path",
        type=str,
        default="results/figures/main/fig3b/fig3b_soc_medae_pure.png",
    )
...
def main():
    ...
    print("[FIGURE] Figure 3b SOC MedAE heatmap")
    ...
    print("[DONE] Complete and pure Figure 3b SOC MedAE heatmaps saved successfully.")
