...
This script:
    1. Rebuilds the test set.
    2. Loads the proposed conditional-flow checkpoint.
    3. Loads the Gaussian calibration-baseline checkpoint.
    4. Runs inference.
    5. Computes calibrated probability residuals.
    6. Searches the best post-hoc calibration temperature.
    7. Plots observed-minus-expected calibration deviation.
    8. Uses a fixed y-axis range from -20% to +20%.
    9. Saves both full and pure versions.
...
Outputs:
    ...
    results/figures/main/fig5c/fig5c_calibration_pure.png
"""
...
from proposed_framework.models.hierarchical_model import (
    Hier3HeadModel,
)
...
FULL_SAVE_NAME = "fig5c_calibration.png"
PURE_SAVE_NAME = "fig5c_calibration_pure.png"
...
def inverse_targets_np(
    ...
    if zscore_normalize:
        if (
            ...
            raise RuntimeError(
                "soc_norm and soh_norm are required."
            )
...
def load_flow_model(
    ...
    if not PROPOSED_CKPT.exists():
        raise FileNotFoundError(
            "Proposed-framework checkpoint not found: "
            f"{PROPOSED_CKPT}"
        )
    ...
    print(
        f"[MODEL] Loaded flow checkpoint: "
        f"{PROPOSED_CKPT}"
    )
...
def load_gaussian_baseline(
    num_classes: int,
    device: str,
) -> torch.nn.Module:
    if not BASELINE_CKPT.exists():
        raise FileNotFoundError(
            "Calibration-baseline checkpoint not found: "
            f"{BASELINE_CKPT}\n"
            "Please run "
            "analysis/train_calibration_baseline.py "
            "first."
        )
    ...
    try:
        ...
        raise RuntimeError(
            "The Gaussian baseline checkpoint is "
            "incompatible with the new "
            "dual-probabilistic-head model. "
            "Re-run "
            "analysis/train_calibration_baseline.py "
            "to regenerate "
    ...
    print(
        f"[MODEL] Loaded Gaussian baseline checkpoint: "
        f"{BASELINE_CKPT}"
    )
...
def build_test_loaders():
    ...
    if (
        ...
        USE_PT_AS_FEATURE
        and "pulse_width_ms" in mtr.columns
    ):
        pt_train = np.log1p(
            mtr[
                "pulse_width_ms"
...
def compute_flow_soc_noise(
    ...
    for (
        ...
        use_pt = model_uses_pt(
            model
        )

        in_features = first_linear_in_features(
            model.head_mat
        )
...
def main() -> None:
    ...
    # =========================================================================
    print(
        "[INFER] Computing conditional-flow "
        "SOC calibration residuals..."
    )
    ...
    print(
        "[INFER] Computing Gaussian-baseline "
        "SOC calibration residuals..."
    )
    ...
    print(
        f"[FULL] {full_save_path}"
    )

    print(
        f"[PURE] {pure_save_path}"
    )
