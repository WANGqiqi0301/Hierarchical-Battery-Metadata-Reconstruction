...
1. Material classification accuracy (%)
2. SOC MedAE (%)
3. SOH MedAE (%)
4. Sample count

The grouping remains based on the complete material-capacity classes
(e.g., LFP_35Ah, LMO_24Ah), while classification performance is evaluated
at the material level (LFP, LMO, NMC).
...
def safe_medae(y_true, y_pred) -> float:
    ...
    return float(
        np.median(
            np.abs(y_pred[valid] - y_true[valid])
        )
    )
...
def build_groupwise_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # -----------------------------------------------------
    # Material classification correctness
    # -----------------------------------------------------
    df["material_correct"] = (
        df["true_material"] == df["pred_material"]
    ).astype(float)
    ...
    # Grouping remains by original material-capacity class
    # -----------------------------------------------------
    agg = (
        df.groupby("true_label", dropna=False)
        .agg(
            material_acc=("material_correct", lambda x: 100.0 * np.nanmean(x)),
            soc_medae=("soc_ae", lambda x: np.nanmedian(x)),
            soh_medae=("soh_ae", lambda x: np.nanmedian(x)),
...
def plot_groupwise_performance(plot_df: pd.DataFrame, save_path: Path):
    ...
    material_vals = plot_df["material_acc"].to_numpy(dtype=float)
    soc_vals = plot_df["soc_medae"].to_numpy(dtype=float)
    soh_vals = plot_df["soh_medae"].to_numpy(dtype=float)
    ...
    material_xlim = nice_upper_limit(
        material_vals,
        min_upper=100,
        pad_ratio=0.22,
        round_base=5,
...
def main():
    ...
    print("\n[DONE] Supplementary Figure 5 generated.")
