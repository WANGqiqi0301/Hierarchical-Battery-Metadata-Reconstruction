...
Reproduce Figure 3b third panel: SOH median APE heatmap.
...
def ensure_soh_error_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "soh_ape_pct" in df.columns:
        df["ape_soh"] = df["soh_ape_pct"].astype(float)
    elif {"soh_true", "soh_pred"}.issubset(df.columns):
        df["ape_soh"] = (
            np.abs((df["soh_true"].astype(float) - df["soh_pred"].astype(float))
                   / (df["soh_true"].astype(float) + 1e-5))
            * 100.0
        )
    else:
        raise RuntimeError("Prediction table must contain soh_ape_pct or soh_true/soh_pred.")

    if "soh_bin" not in df.columns:
        df["soh_bin"] = pd.cut(df["soh_true"].astype(float), bins=10)
...
def generate_prediction_table_from_raw(
    ...
    df["soh_ape_pct"] = (
        np.abs((df["soh_true"] - df["soh_pred"]) / (df["soh_true"] + 1e-5))
        * 100.0
    )
    df["soh_bin"] = pd.cut(df["soh_true"], bins=10)
...
def build_soh_median_pivot(df: pd.DataFrame) -> pd.DataFrame:
...
def plot_soh_median_ape(
    ...
    pivot = build_soh_median_pivot(df)
    mask = pivot.isnull()
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Plot Figure 3b SOH median APE heatmap."
    )
    ...
    parser.add_argument(
        "--fig_out_path",
        type=str,
        default="results/figures/main/fig3b/fig3b_soh_median_ape.png",
    )
...
def main():
    ...
    print("[FIGURE] Figure 3b SOH median APE heatmap")
    ...
    print("[DONE] Figure 3b SOH median APE heatmap saved successfully.")
